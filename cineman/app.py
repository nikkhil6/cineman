...
if os.getenv("GAE_ENV", "").startswith("standard") or os.getenv("CLOUD_RUN_SERVICE"):
    ...
    if database_url:
        ...
        print(
            "⚠️  WARNING: Using temporary SQLite database. Data will not persist between deployments."
        )
        print(
            "⚠️  For production, configure Cloud SQL and set DATABASE_URL environment variable."
        )
else:
...
def chat():
    ...
    try:
        ...
        agent_response = movie_chain.invoke(
            {"user_input": enhanced_user_message, "chat_history": formatted_history}
        )
...
def clear_session():
    ...
    try:
        ...
        session.clear()
        return jsonify(
            {"status": "success", "message": "Session cleared successfully."}
        )
    except Exception as e:
